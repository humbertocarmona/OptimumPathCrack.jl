...
function crackOptimalPaths(g::SimpleDiGraph, org::Int64, dst::Int64,
	...
	dij = LightGraphs.dijkstra_shortest_paths(gr, org, weight_matrix, allpaths = true)

	# Check if there exists a path from `org` to `dst`
	sv = dij.predecessors[dst]
	have_path = size(sv, 1) > 0
	...
	while have_path && n_removed < nMaxRem
		...
		i = dij.predecessors[j][1]  # Move backward along the shortest path
		...
		dij = dijkstra_shortest_paths(gr, org, weight_matrix, allpaths = true)
